...
async function preloadAllImages() {
  ...
  characters.forEach(char => {
    rounds.forEach(r => {
      ...
      if (cat) {
        if (cat.images) cat.images.forEach(img => imageUrls.add(img));
        else if (cat.image) imageUrls.add(cat.image);
      }
    });
    if (char.infoImage) imageUrls.add(char.infoImage);
  });
...
function getCardFrontContent(char, category, index) {
  ...
  const infoSrc = char.infoImage || imageSrc;
...
function renderGrid() {
  ...
  container.innerHTML = characters.map(char => {
    ...
    const currentImg = (categoryData.images && categoryData.images[subIndex]) || categoryData.image || '/images/card_back.png';
    const infoSrc = char.infoImage || currentImg;
